...
{
    // New-CleftMap.ps1 in C#.
    //
    // Builds "Cleftwater" - a 1v1, 512 m map, mask-based terrain.
    //
    ...
    {
        public static string CleftMap(string mapsRoot, string propExtension = ".santp",
            Action<string> log = null, ValidateOptions validate = null, string debugOut = null,
            string folder = "Cleftwater", string mapName = "Cleftwater")
        {
            log ??= _ => { };
            ...
            // The PS threw here unless -Force was passed; the callers always
            // passed it, so this port always overwrites.
            if (Directory.Exists(mapDir)) Directory.Delete(mapDir, true);
            Directory.CreateDirectory(texDir);

            // ------------------------------------------------------------- config ----

            const int size = 512;       // the script's $Size default
            const int seed = 20260822;  // the script's $Seed default
            double M = size;
            ...
            // The PS ran in a fresh process, so everything it did not set carried
            // MapGen's compiled defaults. DeployAll builds several maps in one
            // process, so the knobs this pipeline reads that another build may
            // have changed are pinned back to those defaults explicitly.
            MapGen.SymOrder       = 2;
            MapGen.WaterLevel     = 16.0f;
            MapGen.LandBase       = 21.0f;
            MapGen.BowlDepth      = 0.0f;
            MapGen.MesaPathCount  = 6;
            MapGen.MesaMinSep     = 0.20f;
            MapGen.MesaCentreClear = 0.20f;
            MapGen.MesaInflate    = 0.013f;
            MapGen.MesaBlurRadius = 0.011f;
            MapGen.LaneTargetX    = new float[0];
            MapGen.LaneTargetZ    = new float[0];
            ...
            // The mask pipeline needs the bases and bridges already placed, because its
            // keep-out zones are defined against them.
            log("Building mesa masks (paths -> inflate -> quarter-res -> dilute -> blur -> distance field)...");
            MapGen.BuildMesaField(seed, 13.0f, 8.0f);

            log($"Building heightfield ({size} m, meandering river + pathed mesas)...");
            MapGen.BuildHeight();
            ...
            (double t, double perp)[] alloySideA =
            {
            ...
            foreach (var rp in alloySideA)
            {
                ra++;
                float t = (float)rp.t;
                float perp = (float)rp.perp;
                MapGen.RiverToWorld(t, perp, out float wx, out float wz);
                resolvedA.Add(ResolveSpot(wx, wz, 12.0, 34.0, string.Format("alloy A{0}", ra)));
            }

            var alloyPts = new List<double[]>();
            foreach (var p in resolvedA) alloyPts.Add(new[] { p[0], p[1] });
            foreach (var p in resolvedA) alloyPts.Add(new[] { M - p[0], M - p[1] });
            ...
            {
                ci++;
                if (!MapGen.IsReachable(reach, (float)p[0], (float)p[1])) bad.Add(string.Format("Alloys_{0:D3}", ci));
            }
            ...
            for (int b = 0; b < 2; b++)
            {
                ...
                log(string.Format("  bridge {0}: lowest point {1:N2} m (water {2:N1} m) -> {3}",
                    b + 1, min, MapGen.WaterLevel, state));
            }
            ...
            // Positions alongside the JObj transforms, because the debug render
            // below wants them back and JObj is write-only.
            var bucketPos = new Dictionary<string, List<(float x, float z)>>();
            foreach (var b in treeBps.Concat(rockBps)) { buckets[b] = new List<JObj>(); bucketPos[b] = new List<(float, float)>(); }
            ...
            })
            {
                ...
                for (int k = 0; k < n; k++)
                {
                    ...
                    foreach (var inst in new[] { new[] { x, z, yaw }, new[] { M - x, M - z, yaw + Math.PI } })
                    {
                        ...
                        bucketPos[bp].Add(((float)Math.Round(inst[0], 3), (float)Math.Round(inst[1], 3)));
                    }
                }
                log(string.Format("  {0}: {1} per bank -> {2} total", rocks ? "rocks" : "trees", n, n * 2));
            }
            var propGroups = new List<JObj>();
            var groupPos = new List<(float x, float z)>();  // in propGroups walk order, as the PS debug block reads them
            foreach (var b in treeBps.Concat(rockBps))
            {
                ...
                groupPos.AddRange(bucketPos[b]);
            }

            if (debugOut != null)
            {
                var mx = new List<float>(); var mz = new List<float>(); var mk = new List<int>();
                foreach (var (px, pz) in groupPos) { mx.Add(px); mz.Add(pz); mk.Add(2); }
            ...
            foreach (var p in alloyPts)
            {
                ...
                alloyT.Add(string.Format("Alloys_{0:D3}", ai), T(p[0], y, p[1]));
            }
            ...
            // importantly the height fog. See src\Biomes.ps1. (The PS merged
            // New-MapEnvironment 'Tropical'; the biome never fed these fields.)
            ...
            log("");
            // The PS ran: Test-Sanmap.ps1 -Path $sanmap -CheckTextures
